...
function linea(posi, punti) {
        ...
        for (var i = 0; i < punti.length; i++) {
            if (posi.x > punti[i].x && posi.x < punti[i].x + 20 && posi.y < punti[i].y + 20 && posi.y > punti[i].y) {
                ...
                {
                    console.log(punti[i]);
                    for(var j=0;j<passati.length-1;j++)
                    {
                        if(tieni.x==passati[j].x && tieni.y== passati[j].y  && punti[i].x == passati[j+1].x && punti[i].y == passati[j+1].y)
                        {
                            fai=false;
                        }
                    }
